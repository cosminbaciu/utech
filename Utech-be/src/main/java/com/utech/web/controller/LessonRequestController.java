...
public class LessonRequestController {
    ...
    public ResponseEntity<LessonRequest> addLessonRequest(@RequestBody LessonRequest lessonRequest, @CurrentUser UserPrincipal currentUser){
        ...
        lessonRequest.setCreatedAt(LocalDate.now());
        lessonRequest.setUpdatedAt(LocalDate.now());
        lessonRequest.setSeen(false);

        try{
            Mailer.send("cosmiin.baciu@gmail.com", "LogareCosy1", "cosmiin.baciu@gmail.com", "Ai primit o solicitare de meditatii", "blablabla");
